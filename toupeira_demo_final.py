...
LARGURA_MUNDO = 1600
ALTURA_MUNDO = 1200
AREA_EXPLORAR_OBJETIVO = 0.99

# Jogador
img_toupeira_parada = pygame.image.load("toupeira_parada.png").convert_alpha()
img_toupeira_direita = pygame.image.load("toupeira_direita.png").convert_alpha()
img_toupeira_esquerda = pygame.image.load("toupeira_esquerda.png").convert_alpha()

jogador = pygame.Rect(100, 100, 40, 40)
# Ajusta tamanho, se necessário
img_toupeira_parada = pygame.transform.scale(img_toupeira_parada, (jogador.width, jogador.height))
img_toupeira_direita = pygame.transform.scale(img_toupeira_direita, (jogador.width, jogador.height))
img_toupeira_esquerda = pygame.transform.scale(img_toupeira_esquerda, (jogador.width, jogador.height))
velocidade = 7.5
energia_max = 200
energia = energia_max
subterraneo = False
...
MINIMAPA_TAMANHO = 170
moldura_minimapa = pygame.image.load("moldura_minimapa.png").convert_alpha()
largura_moldura, altura_moldura = moldura_minimapa.get_size()
moldura_minimapa = pygame.transform.scale(moldura_minimapa, (MINIMAPA_TAMANHO , MINIMAPA_TAMANHO ))

minimapa_surface = pygame.Surface((MINIMAPA_TAMANHO-20, MINIMAPA_TAMANHO-40))
...
textura_fundo = pygame.transform.scale(textura_fundo, (LARGURA_MUNDO, ALTURA_MUNDO))

def desenhar_fundo():
    tela.blit(textura_fundo, (-camera_x, -camera_y))
...
def desenhar_minimapa():
    ...
    # Toupeira no minimapa
    jx = int(jogador.centerx * escala_x)
    jy = int(jogador.centery * escala_y)
    pygame.draw.rect(minimapa_surface, (0, 255, 255), (jx - 2, jy - 2, 4, 4))

    # 📦 POSICIONAMENTO da moldura e minimapa
    largura_moldura, altura_moldura = moldura_minimapa.get_size()
    pos_x = LARGURA_TELA - largura_moldura - 10
    pos_y = 10

    # Ajuste de centralização vertical do radar dentro da moldura
    offset_x = (largura_moldura - MINIMAPA_TAMANHO) // 2 +8  # ajuste fino horizontal
    offset_y = (altura_moldura - MINIMAPA_TAMANHO) // 2 + 28  # ajuste fino vertical

    # Desenha o minimapa dentro da moldura
    tela.blit(minimapa_surface, (pos_x + offset_x, pos_y + offset_y))
    tela.blit(moldura_minimapa, (pos_x, pos_y))
...
while rodando:
    ...
    if dx > 0  or dy != 0:
        img_toupeira_atual = img_toupeira_direita
        som_andar.play()
        gerar_particula()
    elif dx < 0 :
        img_toupeira_atual = img_toupeira_esquerda
        som_andar.play()
        gerar_particula()
    else:
        img_toupeira_atual = img_toupeira_parada
    ...
    # Toupeira
    tela.blit(img_toupeira_atual, (jogador.x - camera_x, jogador.y - camera_y))
